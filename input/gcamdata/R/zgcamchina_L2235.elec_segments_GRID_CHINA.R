...
#' module_gcamchina_L2235.elec_segments_GRID
#'
...
#' \code{L2235.Pop_GRID_CHINA}, \code{L2235.GDP_GRID_CHINA}, \code{L2235.LaborForceFillout_GRID_CHINA},
...
module_gcamchina_L2235.elec_segments_GRID <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    province_names_mappings %>%
      ...
      rename(region = grid.region) %>%
      arrange(region) %>%
      repeat_add_columns(tibble(year = MODEL_YEARS)) -> L2235.GDP_GRID_CHINA
    ...
    L2235.GDP_GRID_CHINA %>%
      ...
      add_comments("Value is arbitrary and does not matter; but a value must be read in") %>%
      add_legacy_name("L2235.GDP_GRID_CHINA") %>%
      add_precursors("gcam-china/province_names_mappings") ->
      L2235.GDP_GRID_CHINA
