...
#' module_gcamchina_L232.industry
#'
...
module_gcamchina_L232.industry <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    L202.StubTechProd_in_pulp_energy %>%
      ...
      select(LEVEL2_DATA_NAMES[["DeleteSupplysector"]]) %>%
      distinct ->
      L232.DeleteSupplysector_China_woodpulp

    L232.DeleteSupplysector_CHINAind %>%
      bind_rows(L232.DeleteSupplysector_China_woodpulp) ->
      L232.DeleteSupplysector_CHINAind ## OUTPUT
    ...
    L232.PerCapitaBased_ind %>%
      bind_rows(L2326.PerCapitaBased_aluminum,L2327.PerCapitaBased_paper) %>%
      mutate(region = region) %>% # strip attributes from object
      filter(region == gcamchina.REGION) %>%
      select(LEVEL2_DATA_NAMES[["DeleteFinalDemand"]]) ->
      L232.DeleteFinalDemand_CHINAind  ## OUTPUT
    ...
    L232.DeleteStubCalorieContent_Chinaind %>%
      add_title("China food processing input into food demand") %>%
      add_units("NA") %>%
      add_comments("Generated by deselecting food processing input into food demand for the global China region") %>%
      add_legacy_name("L232.DeleteStubCalorieContent_Chinaind") %>%
