...
#' #29/08/2025 xsl try delete gssp related files
...
module_gcamchina_socioeconomics_SSP_xml <- function(command, ...) {
  ...
  if(command == driver.DECLARE_INPUTS) {
    return(c(paste0("L201.Pop_SSP", SSP_NUMS),
             "L201.GDP_Scen",
             "L201.Pop_GCAMCHINA",
             "L201.GDP_GCAMCHINA",
    ...
    return(c(XML = "socioeconomics_CHINA_SSP1.xml",
    ...
    socioeconomics_CHINA_SSP1.xml <- socioeconomics_CHINA_SSP2.xml <- socioeconomics_CHINA_SSP3.xml <-
      socioeconomics_CHINA_SSP4.xml <- socioeconomics_CHINA_SSP5.xml <- NULL  # silence package check notes
    ...
    # 只处理 SSP1-5
    for(ssp in SSP_NUMS) {
      sspname <- paste0("SSP", ssp)

      # Get population data
      popname <- paste0("L201.Pop_", sspname)
      ...
      china_gdp <- L201.GDP_Scen %>%
        filter(scenario == sspname, region == "China")
      ...
      # Produce output XML
      xmlfn <- paste0("socioeconomics_CHINA_", sspname, ".xml")
      create_xml(xmlfn) %>%
        add_xml_data(L201.Pop_CHINA_SSP, "Pop") %>%
        add_xml_data(L201.GDP_CHINA_Scen %>% select(-scenario), "GDP") %>%
        add_xml_data(L201.GDP_national_updated, "GDP") %>%
        add_xml_data(L201.Pop_national_updated, "Pop") %>%
        add_precursors("L201.GDP_Scen") %>%
        add_precursors("L201.Pop_GCAMCHINA", "L201.GDP_GCAMCHINA",
                       "L201.Pop_national_updated_China", "L201.GDP_national_updated_China") ->
        x
    ...
    # Return SSP XMLs
    return_data(socioeconomics_CHINA_SSP1.xml, socioeconomics_CHINA_SSP2.xml,
                socioeconomics_CHINA_SSP3.xml, socioeconomics_CHINA_SSP4.xml,
                socioeconomics_CHINA_SSP5.xml)
  } else {
