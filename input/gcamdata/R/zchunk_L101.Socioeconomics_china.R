...
#' module_gcam.china_L101.Socioeconomics
#'
...
#' the generated outputs: \code{L101.pcGDP_thous90usd_province}, \code{L101.GDP_mil90usd_province}, \code{L101.Pop_thous_province}. The corresponding file in the
...
#' @author RC Jul 2018
module_gcam.china_L101.Socioeconomics <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    return(c(FILE = "gcam-china/province_names_mappings",
             "L100.population_province_SSP",
             "L100.gdp_province_SSP"))
  } else if(command == driver.DECLARE_OUTPUTS) {
    return(c("L101.pcGDP_thous90usd_province",
    ...
    population_thous_province <- get_data(all_data, "L100.population_province_SSP")
    gdp_mil10usd_province     <- get_data(all_data, "L100.gdp_province_SSP")
    ...
    L101.GDP_mil90usd_province %>%
      left_join_error_no_match(L101.Pop_thous_province, by = c("province", "year")) %>%
    ...
    L101.pcGDP_thous90usd_province %>%
      ...
      same_precursors_as("L101.GDP_mil90usd_province") %>%
      same_precursors_as("L101.Pop_thous_province") ->
      L101.pcGDP_thous90usd_province

    return_data(L101.pcGDP_thous90usd_province, L101.GDP_mil90usd_province, L101.Pop_thous_province)
  } else {
