...
#' module_energy_iron_steel_incelas_SSP_CHINA_xml
#'
...
#' xsl 10/09/2025 xsl temperately delete gSSP,and change 2020 to 2021
module_energy_iron_steel_incelas_SSP_CHINA_xml <- function(command, ...) {
  ...
  if(command == driver.DECLARE_INPUTS) {
    ...
    return(c(XML = "iron_steel_incelas_ssp1_china.xml",
    ...
    iron_steel_incelas_ssp1_china.xml <- iron_steel_incelas_ssp2_china.xml <- iron_steel_incelas_ssp3_china.xml <-
      iron_steel_incelas_ssp4_china.xml <- iron_steel_incelas_ssp5_china.xml<- NULL
    ...
    for(iei in INCOME_ELASTICITY_INPUTS) {
	  ...
	  mutate(income.elasticity = if_else(year  == 2021, L2323.IncomeElasticity_detailed_industry$income.elasticity, income.elasticity)) ->
	  inclome_data
    ...
    return_data(iron_steel_incelas_ssp1_china.xml, iron_steel_incelas_ssp2_china.xml, iron_steel_incelas_ssp3_china.xml, iron_steel_incelas_ssp4_china.xml, iron_steel_incelas_ssp5_china.xml)
  } else {
